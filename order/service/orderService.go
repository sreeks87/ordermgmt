...
func (o *orderSvc) Validate(skus []string, shipid string, orderid string) (bool, error) {
	if len(skus) == 0 {
		return false, errors.New("the list of skus to be updated cant be empty")
	}
	if shipid == "" {
		return false, errors.New("shipment tracking id cant be nil")
	}
	if orderid == "" {
		return false, errors.New("order id cant be nil")
	}
